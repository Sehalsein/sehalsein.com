...
/**
 * Keyboard + gamepad input. Keys are sampled per fixed step so the simulation
 * sees a consistent snapshot; `justPressed` edges are latched between fixed
 * steps so short taps are never missed at low frame rates.
 */
export class Input {
	...
	/** Combined keyboard + gamepad driving input for the player vehicle. */
