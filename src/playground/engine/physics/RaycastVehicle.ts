...
export class RaycastVehicle {
	...
	constructor(physics: PhysicsWorld, stats: VehicleStats, position: THREE.Vector3, yaw: number) {
		...
		const bodyDesc = RAPIER.RigidBodyDesc.dynamic()
			...
			.setAngularDamping(tuning.aero.angularDamping)
			.setCanSleep(false);
	...
	update(physics: PhysicsWorld, input: DriveInput, dt: number): void {
		...
		// --- boost management (slipstreaming multiplies regen) ---
		...
		if (this.boosting) {
			...
			const regen = tuning.stats.boostRegenPerSec * (1 + this.slipstream * tuning.slipstream.regenMultiplier);
		...
		for (const wheel of this.wheels) {
			...
			const rearDrifting = !wheel.steered && (input.handbrake || this.driftAmount > 0.55);
			if (rearDrifting) grip *= this.driftGripScale;
			...
			if (input.handbrake) {
				if (Math.abs(forwardSpeed) <= 1.5) {
					...
					longitudinalImpulse -= forwardWheelSpeed * massShare * 0.45;
				} else if (wheel.powered) {
					longitudinalImpulse -= forwardWheelSpeed * massShare * 0.12;
				}
			}
