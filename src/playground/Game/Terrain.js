...
import { color, Fn, mix, round, smoothstep, texture, uniform, uv, vec2, vec3 } from 'three/tsl'
...
export class Terrain
{
    ...
    setGradient()
    {
        ...
        this.colors = [
            // minimal /playground: green land instead of the folio's orange shore
            { stop: 0.1, value: process.env.NEXT_PUBLIC_MINIMAL ? '#5fa83f' : '#ffa94e' },
    ...
    setNodes()
    {
        // greener grass for the minimal /playground build; folio default for /world
        this.grassColorUniform = uniform(color(process.env.NEXT_PUBLIC_MINIMAL ? '#5fa83f' : '#b8b62e'))
        ...
        {
            // VITE_GRAY debug: flat neutral terrain so the raw geometry is readable
            if(process.env.NEXT_PUBLIC_GRAY)
                return vec3(0.55)

            // /playground: plain default white/gray model color — no colors/textures
