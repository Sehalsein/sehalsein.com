...
export class Fog
{
    constructor()
    {
        ...
        // /playground: plain solid background, effectively no distance fog — raw scene
        if(process.env.NEXT_PUBLIC_MINIMAL)
        {
            this.color = color('#c8c8c8')
            this.game.scene.backgroundNode = this.color
            const far = this.game.view.optimalArea.farDistance
            this.near = uniform(far * 10)
            this.far = uniform(far * 11)
