...
export class Reveal
{
    ...
    updateStep(step)
    {
        // Minimal /playground skips the "press a key to start" intro entirely so the
        // car is fully revealed and drivable the moment the page loads.
        const skip = !!(location.hash.match(/skip/i) || process.env.NEXT_PUBLIC_MINIMAL)
        const speedMultiplier = skip ? 4 : 1
