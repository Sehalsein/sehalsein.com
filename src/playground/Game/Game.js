...
export class Game
{
    ...
    async init()
    {
        ...
        const compressed = !!process.env.NEXT_PUBLIC_COMPRESSED
        ...
        // Load and init RAPIER (compat build inlines the wasm — no bundler config)
        ...
        // Load rest of resources. The minimal /playground build only needs the
        // vehicle, terrain/floor, playground model + a few always-on textures; all
        // the folio-only props/areas/trees are gated behind !MINIMAL so their (heavy)
        // assets are never fetched and can be deleted from /public.
        ...
        const resourcesPromise = this.resourcesLoader.load(
            [
                ...
                // /playground only: extra selectable vehicle body for customization
                ...(minimal ? [ [ 'vehicleOldSchool', `vehicle/oldSchool${compressedModelSuffix}.glb${cb}`, 'gltf' ] ] : []),
                ...
                [ 'terrainTexture',                        `terrain/${minimal ? 'terrainMinimal' : 'terrain'}.${compressedTextureExtension}${cb}`,                                 compressedTextureFormat, (resource) => { resource.flipY = false; } ],
                ...
                // ── folio-only (full /world build) — props, trees, areas, weather ──
                ...(minimal ? [] : [
        ...
        // Tornado is a folio-only weather prop and needs tornadoPathReferencesModel,
        // which the minimal build doesn't load — skip it.
        ...
        // wait() counts frames, not seconds — 3 frames ≈ instant
        this.ticker.wait(3, () =>
        {
