...
export class Rendering
{
    ...
    async setRenderer()
    {
        // Allow forcing the WebGL backend via ?webgl (or #webgl) — WebGPU draws
        // the intro but can gray out on the full world on some GPUs/browsers;
        // TSL compiles to both, so WebGL is the compatible fallback.
        const forceWebGL =
            /(\?|&)webgl\b/i.test(location.search) || /webgl/i.test(location.hash)
    ...
    setPostprocessing()
    {
        ...
        // Minimal /playground: drop the folio's tilt-shift DOF so the car (and the
        // whole scene) is crisp on load — pushing start/end past the max |uv.y-0.5|
        // (0.5) makes the blur strength evaluate to 0 everywhere.
        if(process.env.NEXT_PUBLIC_MINIMAL)
        {
            this.cheapDOFPass.start.value = 0.6
            this.cheapDOFPass.end.value = 0.7
        }
