...
export class View
{
    ...
    setCameras()
    {
        ...
        this.playgroundCameraMode = 'iso'   // /playground: iso | third | first
    ...
    setPlaygroundCamera(mode)
    {
        this.playgroundCameraMode = mode
        this.camera.fov = mode === 'first' ? 70 : mode === 'third' ? 46 : 25
        this.camera.updateProjectionMatrix()
    }

    applyPlaygroundCamera()
    {
        const pv = this.game.physicalVehicle
        if(!pv || !pv.chassis) return

        const body = pv.chassis.physical.body
        const t = body.translation()
        const r = body.rotation()
        const carPos = new THREE.Vector3(t.x, t.y, t.z)
        const fwd = new THREE.Vector3(1, 0, 0).applyQuaternion(new THREE.Quaternion(r.x, r.y, r.z, r.w))
        fwd.y = 0
        ...
        if(this.playgroundCameraMode === 'third')
        {
            cam.position.copy(carPos).addScaledVector(fwd, -9).add(new THREE.Vector3(0, 4.5, 0))
            cam.lookAt(carPos.x + fwd.x * 6, carPos.y + 1.2, carPos.z + fwd.z * 6)
        }
        else
        {
            cam.position.copy(carPos).addScaledVector(fwd, 0.2).add(new THREE.Vector3(0, 1.7, 0))
            cam.lookAt(carPos.x + fwd.x * 25, carPos.y + 1.4, carPos.z + fwd.z * 25)
        }
    ...
    update()
    {
        ...
        // /playground first/third-person camera modes (override the iso rig)
        if(this.cinematic.progress === 0 && (this.playgroundCameraMode === 'third' || this.playgroundCameraMode === 'first'))
            this.applyPlaygroundCamera()
