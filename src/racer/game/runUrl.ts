...
import { allVehicles, getLivery, getVehicle } from "./vehicles/VehicleCatalog";
import { BIOME_IDS } from "../procedural/biomes/biomes";

/**
 * Every run has a shareable URL: /racer/<seed>?biome=…&mode=…
...
/** Parse a race setup from the current location; null on the bare /racer. */
export function parseRunFromLocation(location: Location): RaceSetup | null {
	const match = location.pathname.match(/^\/racer\/([^/]+)\/?$/);
	...
	const vehicle = vehicles.some((v) => v.id === vehicleId) ? (vehicleId as string) : vehicles[0].id;
	return {
		...
		lengthKm: num("length", 1, 1, 4),
		vehicleId: vehicle,
		// getLivery falls back to the car's own default for old/garbled links
		livery: getLivery(params.get("livery"), getVehicle(vehicle)).id,
...
export function runUrl(setup: RaceSetup): string {
	...
	return `/racer/${encodeURIComponent(setup.seed)}?${params.toString()}`;
}
