	...
	update(engine: Engine, dt: number, alpha: number): void {
		...
		for (const id of engine.world.query(PlayerControlled, RenderTransform)) {
			...
			if (vc) {
				...
				// Keep the car pointing up-screen so steering never inverts.
				// Uses the chassis heading, not the velocity: velocity points
				// backwards when reversing, which would spin the camera 180°
				// and invert the controls exactly when you least want it.
				engine.camera.setFollowHeading(vc.vehicle.headingY);
