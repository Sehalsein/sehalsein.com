...
export class Engine {
	...
	simTick = 0;
	/** While paused the simulation is frozen but the scene keeps rendering. */
	paused = false;
	...
	/** Freeze/resume the simulation. Held keys are dropped across the edge. */
	...
	private tick(now: number): void {
		...
		if (this.paused) {
			// drop the backlog: resuming must never replay the paused seconds
			this.accumulator = 0;
			for (const system of this.systems) system.update?.(this, frameDt, 1);
			this.input.endFrame();
