...
export default function Layout(props: PropsWithChildren) {
  return (
    <div
      ...
      style={{
        backgroundImage:
          "url(https://9to5mac.com/wp-content/uploads/sites/6/2024/06/iOS18-Azure-Dark.png)",
      }}
    >
      <StatusBar />
      {props.children}
...
function StatusBar() {
  return (
    <div className="w-full absolute top-0 pt-5 lg:pt-0 lg:bg-gray-900 lg:bg-clip-padding lg:backdrop-filter  lg:backdrop-blur lg:bg-opacity-10 lg:backdrop-saturate-100 lg:backdrop-contrast-100">
...
function TimeView(props: { className?: string; time: string }) {
  return (
    <time className={cn("text-white font-medium", props.className)}>
      {props.time}
    </time>
  );
}
    ...
    <div className="absolute bottom-0 pb-3 h-28 w-full px-3">
      <div className="h-full w-full max-w-sm mx-auto bg-[#232426] rounded-[40px] flex flex-row gap-8 items-center justify-evenly px-5">
        <AppIcon
          ...
          size="sm"
        />
        <AppIcon href={"/os/resume"} icon={<SafariIcon />} size="sm" />
        <AppIcon
          href={`tel:${RESUME_DATA.phone}`}
          icon={<MessageIcon />}
          size="sm"
